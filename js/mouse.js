const colour = "purple"; // Set the sparkle colour
const sparkles = 50;
...
function update_star(i) {
  if (--starv[i] === 25) {
    star[i].style.clipPath = "rect(1px, 4px, 4px, 1px)";
    star[i].style.opacity = "0.6"; // Start fading at mid-life
  }
  if (starv[i]) {
    ...
    if (stary[i] < shigh + sdown) {
      ...
      star[i].style.opacity = Math.max(0, starv[i] / 50); // Gradually decrease opacity
    } else {
...
function newColour() {
  // Generate a purple hue with some variations in brightness
  const c = [];
  c[0] = 128 + Math.floor(Math.random() * 128); // Red range
  c[1] = 0; // Minimal green for purple hues
  c[2] = 128 + Math.floor(Math.random() * 128); // Blue range
